...
namespace DragonLib.IO {
    public static class SpanHelper {
        public static byte ReadByte(Span<byte> buffer, ref int cursor) {
            return buffer[cursor++];
        }

        public static sbyte ReadSByte(Span<byte> buffer, ref int cursor) {
            return (sbyte)buffer[cursor++];
        }

        public static short ReadLittleShort(Span<byte> buffer, ref int cursor) {
            var value = BinaryPrimitives.ReadInt16LittleEndian(buffer[cursor..]);
            cursor += sizeof(short);
            return value;
        }

        public static short ReadBigShort(Span<byte> buffer, ref int cursor) {
            var value = BinaryPrimitives.ReadInt16BigEndian(buffer[cursor..]);
            cursor += sizeof(short);
            return value;
        }

        public static ushort ReadLittleUShort(Span<byte> buffer, ref int cursor) {
            var value = BinaryPrimitives.ReadUInt16LittleEndian(buffer[cursor..]);
            cursor += sizeof(ushort);
            return value;
        }

        public static ushort ReadBigUShort(Span<byte> buffer, ref int cursor) {
            var value = BinaryPrimitives.ReadUInt16BigEndian(buffer[cursor..]);
            cursor += sizeof(ushort);
            return value;
        }

        public static int ReadLittleInt(Span<byte> buffer, ref int cursor) {
            var value = BinaryPrimitives.ReadInt32LittleEndian(buffer[cursor..]);
            cursor += sizeof(int);
            return value;
        }

        public static int ReadBigInt(Span<byte> buffer, ref int cursor) {
            var value = BinaryPrimitives.ReadInt32BigEndian(buffer[cursor..]);
            cursor += sizeof(int);
            return value;
        }

        public static uint ReadLittleUInt(Span<byte> buffer, ref int cursor) {
            var value = BinaryPrimitives.ReadUInt32LittleEndian(buffer[cursor..]);
            cursor += sizeof(uint);
            return value;
        }

        public static uint ReadBigUInt(Span<byte> buffer, ref int cursor) {
            var value = BinaryPrimitives.ReadUInt32BigEndian(buffer[cursor..]);
            cursor += sizeof(uint);
            return value;
        }

        public static long ReadLittleLong(Span<byte> buffer, ref int cursor) {
            var value = BinaryPrimitives.ReadInt64LittleEndian(buffer[cursor..]);
            cursor += sizeof(long);
            return value;
        }

        public static long ReadBigLong(Span<byte> buffer, ref int cursor) {
            var value = BinaryPrimitives.ReadInt64BigEndian(buffer[cursor..]);
            cursor += sizeof(long);
            return value;
        }

        public static ulong ReadLittleULong(Span<byte> buffer, ref int cursor) {
            var value = BinaryPrimitives.ReadUInt64LittleEndian(buffer[cursor..]);
            cursor += sizeof(ulong);
            return value;
        }

        public static ulong ReadBigULong(Span<byte> buffer, ref int cursor) {
            var value = BinaryPrimitives.ReadUInt64BigEndian(buffer[cursor..]);
            cursor += sizeof(ulong);
            return value;
        }

        public static Half ReadLittleHalf(Span<byte> buffer, ref int cursor) {
            var value = BinaryPrimitives.ReadHalfLittleEndian(buffer[cursor..]);
            cursor += sizeof(ushort);
            return value;
        }

        public static Half ReadBigHalf(Span<byte> buffer, ref int cursor) {
            var value = BinaryPrimitives.ReadHalfBigEndian(buffer[cursor..]);
            cursor += sizeof(ushort);
            return value;
        }

        public static float ReadLittleSingle(Span<byte> buffer, ref int cursor) {
            var value = BinaryPrimitives.ReadSingleLittleEndian(buffer[cursor..]);
            cursor += sizeof(float);
            return value;
        }

        public static float ReadBigSingle(Span<byte> buffer, ref int cursor) {
            var value = BinaryPrimitives.ReadSingleBigEndian(buffer[cursor..]);
            cursor += sizeof(float);
            return value;
        }

        public static double ReadLittleDouble(Span<byte> buffer, ref int cursor) {
            var value = BinaryPrimitives.ReadDoubleLittleEndian(buffer[cursor..]);
            cursor += sizeof(double);
            return value;
        }

        public static double ReadBigDouble(Span<byte> buffer, ref int cursor) {
            var value = BinaryPrimitives.ReadDoubleBigEndian(buffer[cursor..]);
            cursor += sizeof(double);
            return value;
        }

        public static decimal ReadLittleDecimal(Span<byte> buffer, ref int cursor) {
            var a = BinaryPrimitives.ReadInt32LittleEndian(buffer[cursor..]);
            var b = BinaryPrimitives.ReadInt32LittleEndian(buffer[(cursor + sizeof(int))..]);
            var c = BinaryPrimitives.ReadInt32LittleEndian(buffer[(cursor + sizeof(int) * 2)..]);
            var d = BinaryPrimitives.ReadInt32LittleEndian(buffer[(cursor + sizeof(int) * 3)..]);
            cursor += sizeof(int) * 4;
            return new decimal(new[] { a, b, c, d });
        }

        public static decimal ReadBigDecimal(Span<byte> buffer, ref int cursor) {
            var a = BinaryPrimitives.ReadInt32BigEndian(buffer[cursor..]);
            var b = BinaryPrimitives.ReadInt32BigEndian(buffer[(cursor + sizeof(int))..]);
            var c = BinaryPrimitives.ReadInt32BigEndian(buffer[(cursor + sizeof(int) * 2)..]);
            var d = BinaryPrimitives.ReadInt32BigEndian(buffer[(cursor + sizeof(int) * 3)..]);
            cursor += sizeof(int) * 4;
        ...
        public static T ReadStruct<T>(Span<byte> buffer, ref int cursor) where T : struct {
            var value = MemoryMarshal.Read<T>(buffer[cursor..]);
        ...
        public static unsafe Array ReadStructArray(Span<byte> buffer, Type T, int count, ref int cursor) {
            ...
            for (var i = 0; i < count; ++i)
                fixed (byte* pin = &buffer[(cursor + refptr)..].GetPinnableReference()) {
        ...
        public static unsafe object? ReadStruct(Span<byte> buffer, Type T, ref int cursor) {
            object? value;
            fixed (byte* pin = &buffer[cursor..].GetPinnableReference()) {
                value = Marshal.PtrToStructure(new IntPtr(pin), T);
            }
        ...
        public static void WriteLittleShort(ref Memory<byte> buffer, short value, ref int cursor) {
            ...
            BinaryPrimitives.WriteInt16LittleEndian(buffer.Span[cursor..], value);
            cursor += sizeof(short);
        }

        public static void WriteBigShort(ref Memory<byte> buffer, short value, ref int cursor) {
            EnsureSpace(ref buffer, cursor + sizeof(short));
            BinaryPrimitives.WriteInt16BigEndian(buffer.Span[cursor..], value);
            cursor += sizeof(short);
        }
        ...
        public static void WriteLittleUShort(ref Memory<byte> buffer, ushort value, ref int cursor) {
            ...
            BinaryPrimitives.WriteUInt16LittleEndian(buffer.Span[cursor..], value);
            cursor += sizeof(short);
        }

        public static void WriteBigUShort(ref Memory<byte> buffer, ushort value, ref int cursor) {
            EnsureSpace(ref buffer, cursor + sizeof(ushort));
            BinaryPrimitives.WriteUInt16BigEndian(buffer.Span[cursor..], value);
            cursor += sizeof(short);
        }
        ...
        public static void WriteLittleInt(ref Memory<byte> buffer, int value, ref int cursor) {
            ...
            BinaryPrimitives.WriteInt32LittleEndian(buffer.Span[cursor..], value);
            cursor += sizeof(int);
        }

        public static void WriteBigInt(ref Memory<byte> buffer, int value, ref int cursor) {
            EnsureSpace(ref buffer, cursor + sizeof(int));
            BinaryPrimitives.WriteInt32BigEndian(buffer.Span[cursor..], value);
            cursor += sizeof(int);
        }
        ...
        public static void WriteLittleUInt(ref Memory<byte> buffer, uint value, ref int cursor) {
            ...
            BinaryPrimitives.WriteUInt32LittleEndian(buffer.Span[cursor..], value);
            cursor += sizeof(int);
        }

        public static void WriteBigUInt(ref Memory<byte> buffer, uint value, ref int cursor) {
            EnsureSpace(ref buffer, cursor + sizeof(uint));
            BinaryPrimitives.WriteUInt32BigEndian(buffer.Span[cursor..], value);
            cursor += sizeof(int);
        }
        ...
        public static void WriteLittleLong(ref Memory<byte> buffer, long value, ref int cursor) {
            ...
            BinaryPrimitives.WriteInt64LittleEndian(buffer.Span[cursor..], value);
            cursor += sizeof(long);
        }

        public static void WriteBigLong(ref Memory<byte> buffer, long value, ref int cursor) {
            EnsureSpace(ref buffer, cursor + sizeof(long));
            BinaryPrimitives.WriteInt64BigEndian(buffer.Span[cursor..], value);
            cursor += sizeof(long);
        }
        ...
        public static void WriteLittleULong(ref Memory<byte> buffer, ulong value, ref int cursor) {
            ...
            BinaryPrimitives.WriteUInt64LittleEndian(buffer.Span[cursor..], value);
            cursor += sizeof(long);
        }

        public static void WriteBigULong(ref Memory<byte> buffer, ulong value, ref int cursor) {
            EnsureSpace(ref buffer, cursor + sizeof(ulong));
            BinaryPrimitives.WriteUInt64BigEndian(buffer.Span[cursor..], value);
            cursor += sizeof(long);
        }

        public static void WriteLittleHalf(ref Memory<byte> buffer, Half value, ref int cursor) {
            EnsureSpace(ref buffer, cursor + sizeof(short));
            BinaryPrimitives.WriteHalfLittleEndian(buffer.Span[cursor..], value);
            cursor += sizeof(short);
        }

        public static void WriteBigHalf(ref Memory<byte> buffer, Half value, ref int cursor) {
            EnsureSpace(ref buffer, cursor + sizeof(short));
            BinaryPrimitives.WriteHalfBigEndian(buffer.Span[cursor..], value);
            cursor += sizeof(short);
        }
        ...
        public static void WriteLittleSingle(ref Memory<byte> buffer, float value, ref int cursor) {
            ...
            BinaryPrimitives.WriteSingleLittleEndian(buffer.Span[cursor..], value);
            cursor += sizeof(float);
        }

        public static void WriteBigSingle(ref Memory<byte> buffer, float value, ref int cursor) {
            EnsureSpace(ref buffer, cursor + sizeof(float));
            BinaryPrimitives.WriteSingleBigEndian(buffer.Span[cursor..], value);
            cursor += sizeof(float);
        }
        ...
        public static void WriteLittleDouble(ref Memory<byte> buffer, double value, ref int cursor) {
            ...
            BinaryPrimitives.WriteDoubleLittleEndian(buffer.Span[cursor..], value);
            cursor += sizeof(double);
        }

        public static void WriteBigDouble(ref Memory<byte> buffer, double value, ref int cursor) {
            EnsureSpace(ref buffer, cursor + sizeof(double));
            BinaryPrimitives.WriteDoubleBigEndian(buffer.Span[cursor..], value);
            cursor += sizeof(double);
        }
        ...
        public static void WriteLittleDecimal(ref Memory<byte> buffer, decimal value, ref int cursor) {
            ...
            BinaryPrimitives.WriteInt32LittleEndian(buffer.Span[cursor..], bits[0]);
            BinaryPrimitives.WriteInt32LittleEndian(buffer.Span[(cursor + sizeof(int))..], bits[1]);
            BinaryPrimitives.WriteInt32LittleEndian(buffer.Span[(cursor + sizeof(int) * 2)..], bits[2]);
            BinaryPrimitives.WriteInt32LittleEndian(buffer.Span[(cursor + sizeof(int) * 3)..], bits[3]);
            cursor += sizeof(int) * 4;
        }
        ...
        public static void WriteBigDecimal(ref Memory<byte> buffer, decimal value, ref int cursor) {
            EnsureSpace(ref buffer, cursor + sizeof(int) * 4);
            var bits = decimal.GetBits(value);
            BinaryPrimitives.WriteInt32BigEndian(buffer.Span[cursor..], bits[3]);
            BinaryPrimitives.WriteInt32BigEndian(buffer.Span[(cursor + sizeof(int))..], bits[2]);
            BinaryPrimitives.WriteInt32BigEndian(buffer.Span[(cursor + sizeof(int) * 2)..], bits[1]);
            BinaryPrimitives.WriteInt32BigEndian(buffer.Span[(cursor + sizeof(int) * 3)..], bits[0]);
            cursor += sizeof(int) * 4;
        }
        ...
        public static void WriteStructArray<T>(ref Memory<byte> buffer, T[] value, ref int cursor) where T : struct {
            ...
            data.CopyTo(buffer.Span[cursor..]);
            cursor += data.Length;
        }
        ...
        public static unsafe void WriteStructArray(ref Memory<byte> buffer, Type T, Array value, ref int cursor) {
            ...
            for (var i = 0; i < value.Length; ++i) {
                ...
                using var destPtr = buffer[(cursor + size * i)..].Pin();
        ...
        public static unsafe void WriteStruct(ref Memory<byte> buffer, Type T, object value, ref int cursor) {
            EnsureSpace(ref buffer, cursor + SizeHelper.SizeOf(T));
            using var destPtr = buffer[cursor..].Pin();
            var ptr = IntPtr.Zero;
