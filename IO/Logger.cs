...
namespace DragonLib.IO
{
    ...
    public static class Logger
    {
            ...
            params object[] arg)
        {
            ...
            if (!EnableVT())
            {
                Log4Bit(color, newline, writer,
                    category, message, arg);
                return;
            }

            Log24Bit(color.AsDOSColor().AsXTermColor().ToForeground(), default, newline,
                writer, category, message,
                arg);
        }
            ...
            params object[] arg)
        {
            ...
            if (!EnableVT())
            {
                Log4Bit(color.AsConsoleColor(), newline, writer,
                    category, message, arg);
                return;
            }

            Log24Bit(color.AsXTermColor().ToForeground(), default, newline,
                writer, category, message,
                arg);
        }
            ...
            params object[] arg)
        {
            ...
            if (!EnableVT())
            {
                Log4Bit(ConsoleColor.Gray, newline, writer,
                    category, message, arg);
                return;
            }

            Log24Bit(color.ToForeground(), default, newline,
                writer, category, message,
                arg);
        }

        public static void Log24Bit(string foreground, string background, bool newLine, TextWriter writer, string category,
        ...
        public static void LogProgress(string message, string pre, string post, double value, XTermColor messageColor,
            XTermColor preColor, XTermColor postColor, XTermColor brickColor, XTermColor processColor, bool showProgressValue,
            XTermColor processValueColor)
        {
            ...
            if (message != LastMessage)
            {
                ...
                LastMessage = message;
                Log24Bit(messageColor, true, Console.Out,
                    null, message);
            }
            ...
            Log24Bit(preColor, false, Console.Out,
                null, pre);
            if (remaining > 0)
            {
                Log24Bit(brickColor, false, Console.Out,
                    null, " [");
                empty = new char[remaining];
                Array.Fill(empty, ' ');
                Array.Fill(empty, '=', 0,
                    (int) Math.Round(remaining * Math.Min(value, 1)));
                Log24Bit(processColor, false, Console.Out,
                    null, string.Join("", empty));
                Log24Bit(brickColor, false, Console.Out,
                    null, "] ");
                ...
                if (showProgressValue && remaining > 6)
                {
                    ...
                    Log24Bit(processValueColor, false, Console.Out,
                        null, valueText);
                    Console.CursorLeft = width - post.Length;
                }
            }

            Log24Bit(postColor, false, Console.Out,
                null, post);
        ...
        public static bool Assert(bool condition, string message = null, params string[] detail)
        {
            ...
            Log24Bit(XTermColor.Purple, true, Console.Error,
                "ASSERT", "Assertion failed at {0}", frame.ToString().Trim());
        ...
        public static void Trace()
        {
            ...
            Log24Bit(XTermColor.HotPink3, true, Console.Error,
