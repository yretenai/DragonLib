...
public static class Logger {
    ...
    private static (string? prefix, string content) FormatMessage(string? category, string? level, string? message) {
        ...
        if (!string.IsNullOrWhiteSpace(category) && ShowTime) {
            parts.Add(DateTimeOffset.UtcNow.ToString("s"));
        }

        if (!string.IsNullOrWhiteSpace(category) && ShowCategory) {
            parts.Add(category);
        }

        if (!string.IsNullOrWhiteSpace(level) && ShowLevel) {
            parts.Add(level);
        }

        if (ShowThread) {
            parts.Add(Environment.CurrentManagedThreadId.ToString("D"));
        }

        var content = message ?? string.Empty;
        var prefix = default(string);
        if (parts.Count > 0) {
            prefix = string.Join(string.Empty, parts.Select(x => $"[{x}]"));
        }

        return (prefix, content);
    }

    public static void Log(XTermColor color,
        bool newline,
        TextWriter writer,
        string? category,
        string? level,
    ...
    public static void Log(string? foreground,
        string? background,
        bool newLine,
        TextWriter writer,
        string? category,
    ...
    public static void PrintVersion(string? category,
        string template = "{0} v{1}",
        Assembly? asm = null,
        string argsTemplate = "Arguments: {0}") {
        asm ??= Assembly.GetEntryAssembly();
        if (asm == null) {
            return;
        }

        Log(XTermColor.White,
            true,
            Console.Error,
            category,
            default,
            string.Format(template, asm.GetName().Name, asm.GetName().Version));
        Log(XTermColor.White,
            true,
            Console.Error,
            category,
            default,
            string.Format(argsTemplate, JsonSerializer.Serialize(Environment.GetCommandLineArgs().Skip(1))));
    }
    ...
    public static void Fatal(string? category, Exception e) {
        Error(category, e.ToString());
    }
    ...
    public static void LogProgress(string message,
        ...
        string post,
        double value,
        XTermColor messageColor,
        XTermColor preColor,
        XTermColor postColor,
        XTermColor brickColor,
        XTermColor processColor,
        bool showProgressValue,
    ...
    public static bool Assert(bool condition, string? message = null, params string[] detail) {
        ...
        Log(XTermColor.Purple,
            true,
            Console.Error,
            default,
            "ASSERT",
            $"Assertion failed at {frame?.ToString().Trim() ?? "unknown location"}");
        ...
        foreach (var line in detail) {
            Log(XTermColor.Purple, true, Console.Error, default, default, "\t -> " + line);
        }
