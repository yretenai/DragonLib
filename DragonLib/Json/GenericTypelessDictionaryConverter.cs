...
public class GenericTypelessDictionaryConverter<TKey, TValue> : JsonConverter<Dictionary<TKey, TValue>>
    where TKey : notnull {
    public GenericTypelessDictionaryConverter(bool overrideIsStringKey, IEnumerable<Type> additonalTypes) {
        if (!IsStringKey && additonalTypes.Any(x => x.IsAssignableFrom(typeof(TKey))) || overrideIsStringKey) {
            IsStringKey = true;
        }
    }
