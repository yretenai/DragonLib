...
public class GenericListConverter<T> : JsonConverter<List<T>> {
    public static bool TryReadValue(ref Utf8JsonReader reader, JsonSerializerOptions options, out T value) {
        ...
        value = (reader.TokenType == JsonTokenType.Null
            ? default
            : (T) JsonSerializer.Deserialize(ref reader, type, options)!)!;
        reader.Read();
        return reader.TokenType == JsonTokenType.EndArray;
    }

    public static void WriteValue(Utf8JsonWriter writer,
        object? value,
        Type? valueType,
    ...
    public override List<T> Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options) {
        ...
        while (reader.Read()) {
            ...
            if (!TryReadValue(ref reader, options, out var value)) {
                throw new JsonException();
            }
