    ...
    public static class DataTypeHelper
    {
        ...
        public static void Preload<T>() where T : struct
        {
            ...
            for (var i = 0; i < values.Length; ++i)
            {
                ...
                if (custom.Any(x => x is FileExtensionAttribute))
                {
                    extension = ((FileExtensionAttribute)custom.First(x => x is FileExtensionAttribute)).Extension;
                }
        ...
        public static bool Matches<T>(Span<byte> span, T magic) where T : struct
        {
            return GetMagicValue<T>(span).Equals(magic);
        }
